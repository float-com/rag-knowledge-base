...
1. 业务流程总编排与外观整合：
   ...
   对外暴露完整的业务用例：文件上传、列表分页、单文档删除、失败重试与分块详情浏览。
...
通俗来讲：
这就是整个文档模块的“业务大总管”——前端发来的上传、看列表、查切片、删文档、重新入库等需求全归它管。
传文件时它负责验真假、算指纹搞秒传、找腾讯云存文件、使唤数据库记账，
并在后台派发 AI 耗时任务；删文档和重跑时它负责把好状态关卡、扫除脏数据，防止系统打架崩溃。
"""
