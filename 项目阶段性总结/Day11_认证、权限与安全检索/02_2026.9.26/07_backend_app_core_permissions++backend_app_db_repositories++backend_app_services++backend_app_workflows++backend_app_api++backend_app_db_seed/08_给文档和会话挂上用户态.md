...
> 📌 **为什么三个 service 各留一份副本**（`RoleService` / `UserService` / `DocumentService`）：
> 三处逻辑完全相同（8 行纯函数），但分属三个互不依赖的模块。
> 抽公共函数反而要新建工具模块、让三个 service 都依赖它 —— **对一个 8 行函数不划算**。
> 真正需要统一的是**行为**，不是**代码位置**，所以在三处都写了同样的说明。
...
### 9.2 🟡 分片上传落地的文档丢标签、丢上传者

```python
# backend/app/services/document_upload_service.py  L287-297
document = Document(
...
├── backend/app/db/repositories/document_repo.py        【修改】+106 · build_permission_filter + 2 处过滤
├── backend/app/db/repositories/conversation_repo.py    【修改】 +87 · 4 个方法加 user_id
├── backend/app/services/document_service.py            【修改】+165 · _normalize_tags + 4 处透传 + update_permission_tags
├── backend/app/services/chat_service.py                【修改】 +87 · stream_answer 加 current_user + 注入 permissions
├── backend/app/services/permission_service.py          【修改】 -25/+… · 常量转发导入 + __all__
├── backend/app/workflows/rag_state.py                  【修改】  +6 · 声明 permissions 字段
├── backend/app/api/routes/documents.py                 【修改】+236 · _viewer_tags + 双来源认证 + PATCH 新端点
├── backend/app/api/routes/chat.py                      【修改】 +25 · 5 个端点加 CurrentUser
├── backend/app/api/routes/evaluations.py               【修改】 +18 · router 级管理员闸门
├── backend/app/api/schemas/documents.py                【修改】 +28 · DocumentRead 补 2 字段 + 新请求模型
└── backend/app/db/seed.py                              【修改】  ±1 · 常量 import 路径改到 core

统计：12 个文件（1 新增 / 11 修改），+721 / -101
