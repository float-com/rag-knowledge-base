...
| 2 | 分片上传落地的文档**丢 `permission_tags` 与 `created_by`** | **标签搬运 ✅ 已修 / `created_by` ❌ 仍未修** → 已独立归档 |
| 3 | 前端 `RequireAdmin` 仍处于**临时停用**状态（`return <Outlet />`） | ✅ **已恢复**（连同 `RequireAuth` 与两处 `isAdmin` Mock，见下方说明） |
...
>
> **当天进展**：
> - **问题 2 的"标签搬运"已修**（`finalize_upload` 补 `permission_tags=normalize_tags(...)`，
>   并把清洗逻辑上收到新建的 `app/core/tags.py`）；实测 18 项全通过
> - **问题 1 的鉴权仍未修**（协议级鉴权设计，涉及"谁在上传"语义，等 Day13）
>
> ⚠️ **不要把"标签修好了"误读成"上传链路安全了"**：
> 现在是"有权限的人上传带标签的文档，标签能正确生效了"，
> 但**任何人都仍然能上传**（只是传上去的标签按他声明值走）。

> 📌 **第 3 条已恢复（2026.09.26）**：前端做界面验证前，把 Day10 遗留的 4 处临时改动全部恢复：
> `RequireAuth.tsx`（真实守卫 + `/auth/me` 同步）、`RequireAdmin.tsx`（403），
> 以及 `BasicLayout.tsx` / `DocumentsPage.tsx` 里两处硬编码的 `isAdmin = true`。
> `tsc -b` + `vite build` 通过，`eslint` 报错数不变。
